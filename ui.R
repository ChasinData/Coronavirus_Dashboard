...
ui <- fluidPage(
  title = "SAFEbuilt Tracker COVID-19 Global Cases",
  tags$head(
  ...
  navbarPage(
    title       = div("SAFEbuilt Tracker COVID-19 Global Cases", style = "padding-left: 10px"),
    collapsible = TRUE,
    fluid       = TRUE,
    ...
    header.append('<div style=\"float:right\"><a target=\"_blank\" href=\"https://github.com/chasindata\"><img src=\"logo.png\" alt=\"alt\" style=\"float:right;width:33px;padding-top:10px;margin-top:-50px;margin-right:10px\"> </a></div>');
    console.log(header)")
    )
  )
)